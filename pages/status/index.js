...
function StatusInfo() {
  ...
  if (error) {
    return (
      <div className="text-red-500 text-sm">❌ Falha ao carregar status</div>
    );
  }

  if (isLoading || !data) {
    return (
      <div className="text-gray-500 text-sm">⏳ Carregando informações...</div>
    );
  }
    ...
    <div className="text-left text-gray-600 text-sm space-y-4 mt-4">
      ...
      <div className="border-t pt-2">
        <div className="flex items-center gap-1">
          <span>🕒</span> Última atualização:{" "}
          <span className="font-mono">{updateAtText}</span>
        </div>
      </div>
    </div>
  );
